import React from "react";
import { Link } from "react-router-dom";



const BlueKotakSewa = () => {
    return( 

<div>
       
       
   <div className="w-4/5 h-96 grid-cols-3 mx-auto rounded-xl border-stone-100 bg-blue-800 mt-8">
    <div className="mx-auto text-center mt-8">
      <p className="font-bold text-2xl text-cyan-50">Sewa Mobil di (Lokasimu!) Sekarang</p>
    </div>
    <div className="mx-auto text-center w-1/3 mt-10">
      <p className="font-bold text-sm text-cyan-50">Lorem, ipsum dolor sit amet consectetur adipisicing elit, sed do elusmod tempor incidunt ut labore et dolore magna aliqua</p>
    </div>
    <div className="mx-auto px-8 py-3 w-48 bg-green-600 rounded-md shadow hover:text-gray-100 hover:bg-green-500 mt-20"> 
    <Link to={'/search'} >
      <p  className="mx-auto  text-gray-100 ">
      Mulai Sewa Mobil</p>
    </Link>
    </div>
   </div>
   

</div>
    
    
    
	
)};

export default BlueKotakSewa;